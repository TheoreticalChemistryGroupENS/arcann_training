...
`"systems_charge"` and `"systems_total_spin"` set the total charge and total spin of each system (one value per system, in the same order as `"systems_auto"`); they must be consistent with the `CHARGE`/`MULTIPLICITY` you set in that system's CP2K template. These are only used when training a PolarMACE model, where they are recorded as the mandatory `charge`/`total_spin` fields on every extracted configuration.
