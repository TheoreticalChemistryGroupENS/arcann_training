...
We **strongly** advise you to create the previous files starting from the templates, as they contain replaceable strings for the key parameters that will be updated by the procedure. These strings all start with `_R_` (*e.g.* `_R_WALLTIME_`), must be kept as-is in your templates, and are filled in automatically by ArcaNN at `prepare` time. The `Slurm` submission files all share a common set of placeholders described in [HPC Configuration](../getting-started/hpc_configuration.md#common-_r_-placeholders-every-slurm-job-script); each step's page ([Exploration](exploration.md#reference-template-placeholders), [Labeling](labeling.md#reference-template-placeholders), Training ([DeePMD-kit](training.md#deepmd-kit-template-placeholders) / [MACE](training.md#mace-template-placeholders)), [Test](test.md#reference-template-placeholders)) lists the additional placeholders specific to that step's job scripts and input-file templates.
