...
**MACE currently requires `exploration_type: "lammps"`.** The `i-PI` and `sander_emle` exploration types described below are only implemented for DeePMD-kit; if `nnp_program` is `"mace"` and a system's `exploration_type` is not `"lammps"`, ArcaNN stops with an error. If you need quantum-nuclei or QM/MM exploration together with MACE, train and explore with DeePMD-kit for now.
...
**Current limitation:** past `check`, i-PI exploration is not currently usable end-to-end. The `deviate` phase requires a prior "rechecked" step that has no corresponding implemented phase in this version of ArcaNN, so `deviate`/`extract` cannot be run for i-PI systems yet. If you need quantum-nuclei exploration today, treat this as a known gap rather than following a documented sequence — check with the developers for the current status before relying on it.
...
During `check`, a run that ArcaNN cannot validate is handled in one of two ways. This applies the same way regardless of `exploration_type` (LAMMPS, i-PI, or `sander_emle`):
