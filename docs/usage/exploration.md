...
**Note on PolarMACE:** MACE models are only supported with `exploration_type: lammps` — ArcaNN has no i-PI support for MACE (i-PI exploration is DeepMD-only) and will abort if you set `exploration_type: i-PI` with `nnp_program: mace`. Within that LAMMPS path, forces are evaluated through `pair_style mace`/`mliap`/`symmetrix/mace`, deployed from your checkpoint with MACE's `mace.cli.create_lammps_model`. This deployment path covers the standard, short-range MACE architecture. Electrostatics-augmented models such as PolarMACE / MACE-POLAR-1 are only documented for ASE usage (`mace.calculators.mace_polar`); loading one through LAMMPS's `pair_style mliap` currently fails upstream with `KeyError: 'fermi_level'` ([ACEsuit/mace#1409](https://github.com/ACEsuit/mace/issues/1409), open). In other words, ArcaNN can prepare a PolarMACE-ready dataset and train a PolarMACE model (see [Labeling](../labeling) for the `systems_charge`/`systems_total_spin` fields), but its built-in exploration step cannot currently drive active learning with that model until LAMMPS support for PolarMACE lands upstream.
